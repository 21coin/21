...
const int BYTES_PER_21XEL = 4;
const int BITS_PER_SAMPLE = 8;

FreedesktopImage::FreedesktopImage(const QImage &img):
    width(img.width()),
    height(img.height()),
    stride(img.width() * BYTES_PER_21XEL),
    hasAlpha(true),
    channels(CHANNELS),
    bitsPerSample(BITS_PER_SAMPLE)
{
    ...
    image.resize(num_pixels * BYTES_PER_21XEL);

    for(unsigned int ptr = 0; ptr < num_pixels; ++ptr)
    {
        image[ptr*BYTES_PER_21XEL+0] = data[ptr] >> 16; // R
        image[ptr*BYTES_PER_21XEL+1] = data[ptr] >> 8;  // G
        image[ptr*BYTES_PER_21XEL+2] = data[ptr];       // B
        image[ptr*BYTES_PER_21XEL+3] = data[ptr] >> 24; // A
    }
}
